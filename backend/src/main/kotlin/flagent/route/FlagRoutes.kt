...
import flagent.api.model.BatchFlagEnabledRequest
import flagent.api.model.CreateFlagRequest
import flagent.api.model.PutFlagRequest
import flagent.api.model.SetFlagEnabledRequest
                ...
                put {
                    ...
                    if (request.ids.isEmpty()) {
                        return@put call.respond(
                            ...
                            mapOf("error" to "ids cannot be empty")
                        )
                    }
                    val updated =
                        flagService.batchSetEnabled(request.ids, request.enabled, updatedBy)
                    request.ids.forEach { id ->
                        AuditLoggerRegistry.logger?.log(
                            call,
                            if (request.enabled) "flag.enabled" else "flag.disabled",
                            "flag:$id",
                            mapOf("id" to id)
                        )
                    }
            ...
            post {
                ...
                try {
                    ...
                    AuditLoggerRegistry.logger?.log(
                        ...
                        "flag.created",
                        "flag:${flag.key}",
                        mapOf("id" to flag.id, "key" to flag.key)
                    )
