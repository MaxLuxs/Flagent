...
import flagent.api.model.EvalDebugLogResponse
...
import flagent.api.model.SegmentDebugLogResponse
...
fun Routing.configureEvaluationRoutes(evaluationService: EvaluationService) {
    route(ApiConstants.API_BASE_PATH) {
        ...
        post("/evaluation/batch") {
            ...
            val hasFlagSelector =
                request.flagIDs.isNotEmpty() || request.flagKeys.isNotEmpty() || request.flagTags.isNotEmpty()
            if (!hasFlagSelector) {
                return@post call.respond(
                    HttpStatusCode.BadRequest,
                    mapOf("error" to "At least one of flagIDs, flagKeys, or flagTags is required")
                )
            }
            if (request.entities.isEmpty()) {
                return@post call.respond(
                    HttpStatusCode.BadRequest,
                    mapOf("error" to "entities must not be empty when flagIDs, flagKeys, or flagTags are provided")
                )
            }
...
private fun mapEvalResultToResponse(result: EvalResult): EvaluationResponse {
