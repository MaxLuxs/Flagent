            ...
            route("/flagent") {
                ...
                post("/evaluate") {
                    ...
                    val request = try {
                        ...
                        val errorMsg = e.message ?: "Invalid request format"
                        call.respond(
                            HttpStatusCode.BadRequest,
                            mapOf("error" to "Invalid request format: $errorMsg")
                        )
                        return@post
                    }
                    ...
                    if (request.flagID == null && request.flagKey == null) {
                        call.respond(
                            HttpStatusCode.BadRequest,
                            mapOf<String, String>("error" to "Either flagID or flagKey must be provided")
                        )
                        return@post
                    }
                ...
                post("/evaluate/batch") {
                    ...
                    if (request.entities.isEmpty()) {
                        call.respond(
                            HttpStatusCode.BadRequest,
                            mapOf<String, String>("error" to "At least one entity must be provided")
                        )
                        return@post
                    }
                    ...
                    if (request.flagIDs.isEmpty() && request.flagKeys.isEmpty() && request.flagTags.isEmpty()) {
                        call.respond(
                            HttpStatusCode.BadRequest,
                            mapOf<String, String>("error" to "At least one of flagIDs, flagKeys, or flagTags must be provided")
                        )
                        return@post
                    }
                    ...
                    if (batchResponse != null) {
                        ...
                        val flagCount =
                            request.flagIDs.size + request.flagKeys.size + request.flagTags.size
