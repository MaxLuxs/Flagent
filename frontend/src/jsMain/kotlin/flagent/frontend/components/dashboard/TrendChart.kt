...
/**
 * Area chart for evaluations trend over time.
 * Uses Chart.js with fill; theme-aware.
 */
@Composable
fun TrendChart(
    ...
    val canvasId = remember { "trend-chart-${currentTimeMillis()}" }
...
private fun createTrendChart(
    ...
    val borderColor = FlagentTheme.Secondary.toString()
    val fillColor = "rgba(20, 184, 166, 0.25)"

    val config = js("""({
        type: 'line',
        data: { labels: [], datasets: [{ label: 'Trend', data: [], borderColor: '', backgroundColor: '', borderWidth: 2, fill: true, tension: 0.35, pointRadius: 0, pointHoverRadius: 4 }] },
        options: {
            responsive: true,
