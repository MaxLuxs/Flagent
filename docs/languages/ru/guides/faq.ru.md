...
### Чем Flagent отличается от LaunchDarkly (и других)?

Flagent — **Kotlin-native**, в первую очередь **self-hosted** и **open source** (Apache 2.0). В отличие от LaunchDarkly (SaaS, коммерческий), вы запускаете Flagent у себя и владеете данными. Возможности: feature flags, A/B эксперименты, постепенные раскаты, kill switches, таргетинг, client-side evaluation (Kotlin Enhanced, Go Enhanced), опциональная аналитика крашей по флагам. В Enterprise: SSO, RBAC, Smart Rollout, Anomaly Detection, on-premise. Подробнее: [Тарифы и издания](pricing-and-editions.md).

### Нужен ли свой сервер?

**Self-hosted (Open Source):** Да — вы запускаете Flagent сами (Docker, Kubernetes или из исходников). Нужен сервер (или кластер) и БД (PostgreSQL, MySQL или SQLite). Нет поминутной оплаты; полный контроль и хранение данных у себя. **Flagent Cloud (SaaS)** в планах, но пока недоступен; когда запустится, можно будет использовать Flagent без своего сервера. См. [Тарифы и издания](pricing-and-editions.md).

### Что входит в Enterprise?

В Enterprise входят: **SSO** (SAML, OAuth/OIDC), **RBAC** (кастомные роли и права), **мультитенантность** и on-premise, **Smart Rollout** (автоматический постепенный раскат и откат), **Anomaly Detection** (алерты и опциональный откат), аналитика **крашей по флагам** и интеграция с Anomaly, **выделенная поддержка**, SLA и кастомная разработка. Контакт: max.developer.luxs@gmail.com. Полное сравнение: [Тарифы и издания](pricing-and-editions.md).
