...
internal typealias FlagentClientAPIChallengeHandler = ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
private var challengeHandlerStore = SynchronizedDictionary<Int, FlagentClientAPIChallengeHandler>()
...
internal class URLSessionRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    internal var taskDidReceiveChallenge: FlagentClientAPIChallengeHandler?
    ...
    @discardableResult
    override internal func execute(_ apiResponseQueue: DispatchQueue = FlagentClientAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
    ...
    internal func buildHeaders() -> [String: String] {
        ...
        for (key, value) in FlagentClientAPI.customHeaders {
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.FlagentClient.\(UUID().uuidString)")
                }
